...
global red_team, blue_team, time_count, time_step
...
class Referee:
    ...
    def toss_a_coin_if_needed(self, attribute):  # attribute should be either "side_left" or "kickoff"
        ...
        if hasattr(self.game, attribute):
            if getattr(self.game, attribute) == 'red':
                ...
                error(f'Unsupported value for "{attribute}" in game.json file: {getattr(self.game, attribute)}, using "random".')
    ...
    def game_controller_receive(self):
        ...
        if previous_state != self.game.state.game_state:
            info(f'New state received from GameController: {self.game.state.game_state}.')
            if self.game.wait_for_state is not None:
                if 'STATE_' + self.game.wait_for_state != self.game.state.game_state:
                    warning(f'Received unexpected state from GameController: {self.game.state.game_state} ' +
                            f'while expecting {self.game.wait_for_state}')
                else:
                    info(f"State has succesfully changed to {self.game.wait_for_state}")
        ...
        if self.game.state.game_state == 'STATE_PLAYING' and \
            ...
            if self.game.in_play is None and self.game.phase == 'KICKOFF':
                ...
                self.game.in_play = time_count
                self.game.ball_last_move = time_count
        ...
        if secondary_state[0:6] == 'STATE_' and secondary_state[6:] in GAME_INTERRUPTIONS:
            ...
            delay = (time_count - self.game.interruption_step_time) / 1000
            if step == 0 and self.game.interruption_step != step:
                self.game.interruption_step = step
                self.game.interruption_step_time = time_count
            ...
            if msg_start in GAME_INTERRUPTIONS:
                if 'ABORT' in message or 'EXECUTE' in message:
                    ...
                    if 'READY' in message:
                        ...
                        self.game.wait_for_sec_phase = 0
                info(f"Waiting for secondary state: {self.game.wait_for_sec_state}:{self.game.wait_for_sec_phase}")
                ...
                if point[2] > self.game.field.turf_depth:  # not a contact with the ground
                    if not early_game_interruption and point in self.game.ball.contact_points:  # ball contact
                        ...
                        if self.game.ball_first_touch_time == 0:
                            self.game.ball_first_touch_time = time_count
                        self.game.ball_last_touch_time = time_count
                        ...
                        if self.game.ball_last_touch_team != color or self.game.ball_last_touch_player_number != int(number):
                            ...
                            self.game.ball_last_touch_time_for_display = time_count
                        ...
                        elif time_count - self.game.ball_last_touch_time_for_display >= 1000:  # dont produce too many touched messages
                            self.game.ball_last_touch_time_for_display = time_count
    ...
    def update_robot_contacts(self):
        self.game.forceful_contact_matrix.clear(time_count)
    ...
    def forceful_contact_foul(self, team, number, opponent_team, opponent_number, distance_to_ball, message):
        ...
        info(f"Ball in play: {self.game.in_play}, foul far from ball: {foul_far_from_ball}")
    ...
    def game_interruption_touched(self, team, number):
        ...
        if opponent:
            ...
            info(f"Ball touched by opponent, retaking {GAME_INTERRUPTIONS[self.game.interruption]}")
            info(f"Reset interruption_countdown to {self.game.interruption_countdown}")
            self.game_controller_send(f'{self.game.interruption}:{self.game.interruption_team}:RETAKE')
        else:
            self.game.in_play = time_count
            info(f"Ball touched before execute, aborting {GAME_INTERRUPTIONS[self.game.interruption]}")
    ...
    def next_penalty_shootout(self):
        ...
        self.flip_sides()
        info(f'fliped sides: game.side_left = {self.game.side_left}')
    ...
    def interruption(self, interruption_type, team=None, location=None, is_goalkeeper_ball_manipulation=False):
            ...
            if inside_penalty_area and own_side:
                if is_goalkeeper_ball_manipulation:
                    ...
                    if abs(location[1]) > dy:
                        ...
                        moved = True
                    if moved:
                        info(f'Moved the ball on the penalty line at {self.game.ball_kick_translation}')
        ...
        self.game.interruption_countdown = self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0
        info(f'Interruption countdown set to {self.game.interruption_countdown}')
        if not team:
        ...
        self.reset_ball_touched()
        info(f'Ball not in play, will be kicked by a player from the {self.game.ball_must_kick_team} team.')
    ...
    def setup(self):
        ...
        if self.game.type not in ['NORMAL', 'KNOCKOUT', 'PENALTY']:
            error(f'Unsupported game type: {self.game.type}.')
            self.clean_exit()
        self.game.penalty_shootout = self.game.type == 'PENALTY'
        info(f'Minimum real time factor is set to {self.game.minimum_real_time_factor}.')
        if self.game.minimum_real_time_factor == 0:
            ...
            info(f'Simulation will guarantee a maximum {1 / self.game.minimum_real_time_factor:.2f}x speed for each time step.')
        ...
        if host != '127.0.0.1' and host != self.game.host:
            warning(f'Host is not correctly defined in game.json file, it should be {host} instead of {self.game.host}.')
        ...
        try:
            ...
            info(f'Game type is {self.game.type}.')
            info(f'Red team is "{red_team["name"]}", playing on {"left" if self.game.side_left == self.game.red.id else "right"} side.')
            info(f'Blue team is "{blue_team["name"]}", playing on {"left" if self.game.side_left == self.game.blue.id else "right"} side.')
            ...
            if self.game.penalty_shootout:
                info(f'{"Red" if self.game.kickoff == self.game.red.id else "Blue"} team will start the penalty shoot-out.')
    ...
    def main_loop(self):
        try:
            ...
            while self.supervisor.step(time_step) != -1 and not self.game.over:
                ...
                if self.game.state.game_state == 'STATE_PLAYING' and not self.is_early_game_interruption():
                    ...
                    if self.game.in_play is None:
                        ...
                        if self.game.ball_first_touch_time != 0:
                            ...
                            if d > self.config.BALL_IN_PLAY_MOVE:
                                info(f'{self.game.ball_kick_translation} {self.game.ball_position}')
                        ...
                        if time_count - self.game.ball_last_move > self.config.DROPPED_BALL_TIMEOUT * 1000:
                            self.dropped_ball()
                        if self.game.ball_left_circle is None and self.game.phase == 'KICKOFF':
                            if distance2(self.game.ball_kick_translation, self.game.ball_position) > self.game.field.circle_radius + self.game.ball_radius:
                                self.game.ball_left_circle = time_count
                    ...
                    if self.game.penalty_shootout:
                        ...
                        if ball_in_goal_area and np.linalg.norm(ball_vel) < self.config.STATIC_SPEED_EPS:
                            info(f"Ball stopped in goal area at {self.game.ball_position}")
                            self.next_penalty_shootout()
                    ...
                    if self.previous_seconds_remaining != self.game.state.seconds_remaining:
                        ...
                        if self.game.state.game_state != "STATE_FINISHED" and self.game.state.seconds_remaining <= 0 and \
                            ...
                            info(f"Sending FINISH because seconds remaining = {self.game.state.seconds_remaining}")
                            self.game_controller_send('STATE:FINISH')
                    ...
                    if (self.game.interruption_countdown == 0 and self.game.ready_countdown == 0 and
                        ...
                        info(f'Ball left the field at ({self.game.ball_position[0]} {self.game.ball_position[1]} {self.game.ball_position[2]}) after '
                        ...
                        if scoring_team:
                            ...
                            if not self.game.can_score:
                                if self.game.phase == 'KICKOFF':
                                    ...
                                    info(f'Invalidated direct score in {goal} goal from {GAME_INTERRUPTIONS[self.game.phase]}.')
                                ...
                                info(f'Score in {goal} goal by {self.game.ball_last_touch_team} player {self.game.ball_last_touch_player_number}')
                                if self.game.penalty_shootout:
                                    self.game.penalty_shootout_goal = True
                                    self.next_penalty_shootout()
                                else:
                                    self.game.ready_countdown = self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0
                                    info(f"Ready countdown was set to {self.game.ready_countdown}")
                                    self.kickoff()
                            elif not right_way:  # own goal
                                self.game_controller_send(f'SCORE:{scoring_team}')
                                info(f'Score in {goal} goal by {self.game.ball_last_touch_team} player ' +
                ...
                if self.game.minimum_real_time_factor != 0:
                    ...
                    t = time.time()
                    delta_time = previous_real_time - t + self.game.minimum_real_time_factor * time_step / 1000
                    if delta_time > 0:
            ...
            if not self.game.over:  # for some reason, the simulation was terminated before the end of the match (may happen during tests)
                ...
                if self.game.state.teams[0].score > self.game.state.teams[1].score:
                    ...
                    winner = 1
                    loser = 0
                info(f'The score is {self.game.state.teams[winner].score}-{self.game.state.teams[loser].score}.')
                if self.game.state.teams[0].score != self.game.state.teams[1].score:
                    info(f'The winner is the {self.game.state.teams[winner].team_color.lower()} team.')
